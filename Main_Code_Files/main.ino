...
int SERVO_PIN = 42;                      // Using GPIO 42 for the servo signal
bool increasing = true;                  // Direction flag for the servo
unsigned long previousMillis_servo = 0;  // Timer for non-blocking servo motion
const unsigned long interval_servo = 5;
...
uint8_t wifi_request_count = 0;

int handlet_count = 0;  // Initializing a counter
...
PositionData waypoints[] = { { -1000, 0, 0 }, { -1000, -900, 0 }, { -3600, -900, 0 }, { -4400, -600, 0 }, { -2300, -400, 0 }, { -4200, -600, 0 }, { -4500, -1100, 0 } };
// BLUE PositionData waypoints[] = { { -1000, 0, 0 }, { -1000, -900, 0 }, { -3600, -900, 0 }, { -4400, -600, 0 }, { -2300, -400, 0 }, { -4200, -600, 0 }, { -4500, -1100, 0 } };
...
void IRAM_ATTR encoderISR0() {
  ticksPerInterval[0] += motorDir[0];
}
void IRAM_ATTR encoderISR1() {
  ticksPerInterval[1] += motorDir[1];
}
void IRAM_ATTR encoderISR2() {
  ticksPerInterval[2] += motorDir[2];
}
void IRAM_ATTR encoderISR3() {
  ticksPerInterval[3] += motorDir[3];
}
...
void runServo() {
  ...
  for (int i = 0; i <= 150; i += 1) {
    servo.write(i);  // Move servo to the position in variable 'pos'
    delay(4);
  }
...
void setup() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    attachInterrupt(digitalPinToInterrupt(encoderPIN1[i]), i == 0 ? encoderISR0 : (i == 1 ? encoderISR1 : (i == 2 ? encoderISR2 : encoderISR3)), CHANGE);
  }
...
void loop() {
  ...
  if (digitalRead(ESPIN) == HIGH){
    Serial.println("Health reached 0, Stopping Bulma!");
    motorControl[0] = 0;  // Stop
    motorControl[1] = 0;
    return;
  }
